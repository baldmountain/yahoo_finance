...
defmodule YahooFinanceTest do
  ...
  test "yahoo finance" do
    ...
    hq = YahooFinance.get_historical_quotes_using_dates("FB", start_date, end_date, 3000)
    assert Enum.count(hq) > 0
    assert Enum.at(hq, 0).symbol == "FB"
    assert Enum.at(hq, 0).date != nil
    assert YahooFinance.BaseQuote.valid? Enum.at(hq, 0)

    hq = Enum.at YahooFinance.get_historical_quotes_using_dates("fb", {2013,7,3}, {2013,7,3}), 0
    assert to_string(hq) == "symbol: FB date: 2013-07-03 open: 24.22 high: 24.71 low: 24.15 close: 24.52 volume: 10404400 adjClose: 24.52"
  end
